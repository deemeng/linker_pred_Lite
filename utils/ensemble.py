...
# Prediction
def get_true_pred(model, entity_id, feature_type, path_protTrans):
    '''
    ...
    if feature_type=='protTrans':
        ...
        print(f"{bcolors.WARNING}Warning: wrong feature Type: {feature_type}{bcolors.ENDC}\nProtTrans only.")
...
def ensemble_predict(models, list_modelInfo, entity_id, path_protTrans):
    '''
    ...
    with torch.no_grad():
        for model, model_info in zip(models, list_modelInfo):
            ...
            pred = get_true_pred(model, entity_id, feature_type, path_protTrans)
